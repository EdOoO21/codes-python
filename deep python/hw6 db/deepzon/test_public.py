...
def test_get_total_quantity_per_customer(order_db):
    ...
    assert any(total["_id"] == "Alice" and total["total_quantity"]
               == 5 for total in totals)
    assert any(total["_id"] == "Bob" and total["total_quantity"]
               == 1 for total in totals)
...
def test_get_total_sales_by_product(order_db):
    ...
    assert any(sale["_id"] == "Laptop" and sale["total_sales"]
               == 3000.0 for sale in sales)
    assert any(sale["_id"] == "Mouse" and sale["total_sales"]
               == 75.0 for sale in sales)
...
def test_get_average_order_value_per_customer(order_db):
    ...
    assert any(
        avg["_id"] == "Alice" and pytest.approx(
            avg["average_order_value"],
            0.1) == 525.0 for avg in averages)
...
def test_get_order_count_by_status(order_db):
    ...
    assert any(status["_id"] == "new" and status["order_count"]
               == 1 for status in status_counts)
    assert any(status["_id"] == "shipped" and status["order_count"]
               == 2 for status in status_counts)
