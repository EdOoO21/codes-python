...
def test__build_file(cpython_directory: Path) -> None:
    ...
    assert len(
        tree.body) == 1, "Файл `BUILD.bazel` должен состоять из одной инструкции"
    ...
    assert isinstance(
        cc_import, ast.Call), "Файл `BUILD.bazel` не содержит вызов `cc_import`"
    ...
    assert isinstance(
        visibility_node, ast.List), "Атрибут 'visibility' должен быть списком строк"
    ...
    assert isinstance(
        shared_lib_node, ast.Constant), "Атрибут 'shared_library' должен быть строкой"
    ...
    assert isinstance(
        includes_node, ast.List), "Атрибут 'includes' должен быть списком строк"
    ...
    assert isinstance(
        headers_node, ast.List), "Атрибут 'hdrs' должен быть списком строк"

    headers: list[str] = [
        node.value for node in headers_node.elts]  # type: ignore
    ...
    if not all(cpython_directory.joinpath(header).exists()
