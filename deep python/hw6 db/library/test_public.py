...
def test_get_books_by_author(library):
    ...
    assert books[0][1] == "Harry Potter and the Chamber of Secrets", \
        "Название книги неверное."
...
def test_get_available_books(library):
    ...
    assert available_books[0][1] == "Harry Potter and the Goblet of Fire", \
        "Название книги неверное."
...
def test_search_book(library):
    ...
    assert books[0][1] == "Harry Potter and the Half-Blood Prince", \
        "Название найденной книги неверное."
...
def test_sql_injection_in_search_book(library):
    ...
    assert result == [], \
        "SQL-инъекция сработала, что недопустимо. Или выпала системная ошибка"
