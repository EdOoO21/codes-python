...
def test_link_client_and_membership(db):
    ...
    cursor.execute(
        'SELECT * FROM t_client_membership WHERE \
            client_id = ? AND membership_id = ?',
        (client_id,
         membership_id))
    link = cursor.fetchone()
    cursor.close()
    assert link is not None, \
        "Связь между клиентом и абонементом не была создана."
...
def test_get_client_membership(db):
    ...
    assert ("Gym Access",) in memberships, \
        "Абонемент 'Gym Access' отсутствует."
    assert ("Swimming Pool",) in memberships, \
        "Абонемент 'Swimming Pool' отсутствует."
