...
class TestShip(unittest.TestCase):
    ...
    def test_ship_hit(self):
        ...
        # Проверка, что корабль еще не потоплен
        self.assertFalse(ship.is_sunk())

        ship.hit()
        ship.hit()
        # После трёх попаданий, корабль должен быть потоплен
...
class TestBoard(unittest.TestCase):
    ...
    def test_invalid_ship_placement(self):
        ship = Ship(4)
        placed = self.board.place_ship(
            ship, 0, 8, horizontal=True)  # Корабль выходит за границу
    ...
    def test_all_ships_sunk(self):
        ...
        # Все корабли должны быть потоплены
