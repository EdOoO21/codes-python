...
def test__read_write(adapter: StorageAdapter) -> None:
...
def test__overwrite__commited_step(adapter: StorageAdapter) -> None:
...
def test__get__not_exists(adapter: StorageAdapter) -> None:
...
def test__delete__uncommited_step(adapter: StorageAdapter) -> None:
...
def test__clear__commited_step(adapter: StorageAdapter) -> None:
...
def test__clear__update(adapter: StorageAdapter) -> None:
...
def test__rollback__clear(adapter: StorageAdapter) -> None:
...
def test__read_write__magic_methods(adapter: StorageAdapter) -> None:
    """Тест цикла 'запись - чтение'."""
    adapter[KEY] = VALUE
    adapter.commit()
...
def test__overwrite__commited_step__magic_methods(adapter: StorageAdapter) -> None:
    """Тест перезаписи при промежуточных коммитах."""
...
def test__get__not_exists__magic_methods(adapter: StorageAdapter) -> None:
    """Тест метода `get`, если значения по ключу нет."""
    value = adapter[KEY]
...
def test__delete__uncommited_step__magic_methods(adapter: StorageAdapter) -> None:
    """Тест метода `delete` без промежуточных коммитов."""
    adapter[KEY] = VALUE
    del adapter[KEY]
    adapter.commit()
...
def test__clear__commited_step__magic_methods(adapter: StorageAdapter) -> None:
    """Тест метода `clear` при промежуточных коммитах."""
    adapter[KEY] = VALUE
...
def test__clear__update__magic_methods(adapter: StorageAdapter) -> None:
    """Тест метода `clear`, если до коммита, но после чистки была запись."""
    adapter[KEY] = VALUE
    adapter.clear()
    adapter[ANOTHER_KEY] = ANOTHER_VALUE
...
def test__rollback__clear__magic_methods(adapter: StorageAdapter) -> None:
    """Тест метода `rollback` для `clear`."""
    adapter[KEY] = VALUE
    adapter.commit()
...
def test__transaction__context(adapter: StorageAdapter) -> None:
...
def test__transaction__context_and_self(adapter: StorageAdapter) -> None:
    """Тест контекстного менеджера."""
    with adapter as context:
        adapter.update(KEY, VALUE)
...
def test__transaction__clear(adapter: StorageAdapter) -> None:
    """Тест контекстного менеджера."""
    with adapter:
        adapter.update(KEY, VALUE)
        adapter.update(ANOTHER_KEY, ANOTHER_VALUE)
...
def test__transaction__rollback(adapter: StorageAdapter) -> None:
...
def test__transaction__reraise(adapter: StorageAdapter) -> None:
    """Тест контекстного менеджера при исключении."""
...
def test__virus__delete_file(sandbox: Path, adapter: StorageAdapter) -> None:
    """Вирус удалил файл."""
    with adapter:
        adapter.update(KEY, VALUE)
...
def test__virus__delete_storage_directory_after(sandbox: Path, adapter: StorageAdapter) -> None:
...
def test__virus__storage_directory_as_file(sandbox: Path, adapter: StorageAdapter) -> None:
...
def test__algorithm__shared_read_write(sandbox: Path) -> None:
...
def test__algorithm__shared_clear(sandbox: Path) -> None:
    """Алгоритм разделяет память между адаптерами."""
    lhs = StorageAdapter(_storage_directory=sandbox)
    rhs = StorageAdapter(_storage_directory=sandbox)
