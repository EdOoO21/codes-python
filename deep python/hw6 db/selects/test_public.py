...
from selects import DataBaseHandler


@pytest.fixture(scope="session")
def handler() -> tp.Generator[DataBaseHandler, None, None]:
    with zipfile.ZipFile(Path(__file__).parent / 'chinook.zip', 'r') \
            as zip_ref:
...
def test_track_by_type(handler: DataBaseHandler, case: TrackByTypes) -> None:
    result = handler.get_tracks_of_given_genres(
        case.media_types, case.number_of_tracks)
    assert result == case.results


@pytest.mark.parametrize('case', TracksPlaylistsCases)
def test_track_by_playlist_name(
        handler: DataBaseHandler,
        case: TracksByPlaylistName) -> None:
    result = handler.get_tracks_that_belong_to_playlist_found_by_name(
        case.name_needle)
    assert result == case.results
